...
public class ExcelUtil<T> {
     ...
     */
    public boolean exportExcel(List<T> list, String fileName, HttpServletResponse response) {
        // 得到所有定义字段
        Field[] allFields = clazz.getDeclaredFields();
        ...
        for (int index = 0; index <= sheetNo; index++) {
            ...
            if (sheetNo == 0) {
                workbook.setSheetName(index, fileName);
            } else {
                // 设置工作表的名称.
                workbook.setSheetName(index, fileName + index);
            }
            HSSFRow row;
            HSSFCell cell; // 产生单元格
